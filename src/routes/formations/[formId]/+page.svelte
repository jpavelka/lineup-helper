<script>
  ...
  function addPosition() {
    formation.positions = [...formation.positions, { 
      id: crypto.randomUUID(), name: 'NEW', group: 'MID', x: 50, y: 50 
    }];
    scheduleSave();
  }
</script>
...
<style>
  ...
  .field-container { position: relative; width: 100%; aspect-ratio: 2 / 3; background: #166534; border: 3px solid #fff; border-radius: 8px; overflow: hidden; touch-action: none; }
  ...
  .position-node { position: absolute; transform: translate(-50%, -50%); background: #334155; border: 2px solid #94a3b8; color: white; width: 3.5rem; height: 3.5rem; border-radius: 50%; display: flex; flex-direction: column; justify-content: center; align-items: center; cursor: grab; user-select: none; box-shadow: 0 4px 6px rgba(0,0,0,0.3); transition: transform 0.1s, box-shadow 0.1s; }
  ...
  .node-label { font-weight: bold; font-size: 1rem; }
  .node-group { font-size: 0.7rem; color: #cbd5e1; }
  .muted { color: #94a3b8; }
</style>